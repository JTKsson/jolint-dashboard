...
export default function WeeklyInclusionScoreCard() {
  ...
  const getPreviousISOWeek = (currentISOWeek) => {
    ...
    const previousWeekStartDate = new Date(
      currentDate - days * 24 * 60 * 60 * 1000 - 7 * 24 * 60 * 60 * 1000,
    )
    const previousWeek =
      Math.floor((previousWeekStartDate.getDate() - 1) / 7) + 1
    return `${year}-W${String(previousWeek).padStart(2, '0')}`
  }

  useEffect(() => {
    const currentWeek = getCurrentISOWeek()
    const currentData = inclusionData.find(
      (dataItem) => dataItem['ISO_Week'] === currentWeek,
    )

    const previousWeek = getPreviousISOWeek(currentWeek)
    const previousData = inclusionData.find(
      (dataItem) => dataItem['ISO_Week'] === previousWeek,
    )
    ...
    setUniqueWeeks(
      Array.from(
        new Set(inclusionData.map((dataItem) => dataItem['ISO_Week'])),
      ),
    )
  }, [])

  const handleWeekChange = (selectedWeek) => {
    const selectedDataItem = inclusionData.find(
      (dataItem) => dataItem['ISO_Week'] === selectedWeek,
    )
  ...
  const renderTopRowContainer = (label, infoKey, tooltip) => (
    <div className={styles.topRowContainer}>
      ...
      {selectedData && previousWeekData && (
        <ScoreChangeArrows
          lastMonth={Math.floor(
            selectedData[infoKey] - previousWeekData[infoKey],
          )}
        />
      )}
    </div>
    ...
    <div className={styles.main}>
      ...
      <div className={styles.rowOne}>
        {renderTopRowContainer(
          'Team interactions',
          'Team Inclusion',
          'Evaluates the frequency and quality of communications between team members.',
        )}
      ...
      <div className={styles.rowTwo}>
        {renderTopRowContainer(
          ...
          'Measures the engagement between different departments or functions, showing how well teams are working cross-functionally.',
        )}
        {renderProgressBar(
          'Cross-functional interaction',
          'Cross-Functional Inclusion',
      ...
      <div className={styles.rowThree}>
        {renderTopRowContainer(
          'Informal influence',
          'Informal Influence',
          'Unofficial impact individuals have within the organisation.',
        )}
      ...
      <div className={styles.rowFour}>
        {renderTopRowContainer(
          'Work habits',
          'Work Habits',
          'Assesses the flexibility and inclusivity of work routines, such as meeting times, length of days, and work on weekends.',
        )}
